-- word_similarity() (finds the best-matching word-boundary substring of the
-- longer text) discriminates much better than similarity() for a short
-- search term against a long product name/description — similarity('Bottled
-- Water 75cl', 'Wather') scored only 0.18 (diluted by the full string's
-- trigram count) vs word_similarity('Wather', 'Bottled Water 75cl') at
-- 0.44, while a genuine non-match still scores 0 either way. Verified live
-- against test data before this fix — "Wather" for "Water" returned zero
-- rows under the old similarity()-based threshold.
