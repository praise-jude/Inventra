-- Closes a known gap from Phase F: the Reports page/screen is gated on
-- both platforms at the UI layer (canViewReports()), but the underlying
-- RPCs themselves had no premium check of their own — a Free-tier org
-- could still get real data back from a direct .rpc() call with a valid
-- session. These are all `language sql` (not plpgsql), so a non-premium
-- caller gets an empty/zero result rather than a raised exception —
-- consistent with how RLS itself filters rows rather than erroring.
-- org_is_premium() is security definer (from the freemium_premium_plans
-- migration), safe to call here regardless of caller RLS.

create or replace function get_inventory_valuation(p_warehouse_id uuid default null)
returns table(product_id uuid, name text, sku text, warehouse_id uuid, warehouse_name text, qty_on_hand integer, cost_price numeric, sell_price numeric, inventory_value numeric, expected_profit numeric)
language sql stable as $$
  ...
  where p.org_id = current_org_id() and org_is_premium() and p.archived_at is null
    and (p_warehouse_id is null or p.warehouse_id = p_warehouse_id)
  order by inventory_value desc
$$;

create or replace function get_profit_loss(p_from date, p_to date, p_warehouse_id uuid default null, p_product_id uuid default null)
returns jsonb
language sql stable as $$
  ...
  from (
    ...
    where m.org_id = current_org_id() and org_is_premium() and m.type = 'sale' and m.created_at::date between p_from and p_to
  ...
  cross join (
    ...
    where org_id = current_org_id() and org_is_premium() and incurred_at between p_from and p_to
  ) exp
$$;

create or replace function get_sales_by_branch(p_from date, p_to date)
...
create or replace function get_sales_by_period(p_from date, p_to date, p_granularity text, p_warehouse_id uuid default null)
...
language sql stable as $$
  with rev as (
    ...
    from sales s
    where s.org_id = current_org_id() and org_is_premium() and s.created_at::date between p_from and p_to
...
create or replace function get_sales_by_product(p_from date, p_to date, p_warehouse_id uuid default null)
...
create or replace function get_sales_by_staff(p_from date, p_to date, p_warehouse_id uuid default null)
...
create or replace function get_sales_summary(p_from date, p_to date, p_warehouse_id uuid default null)
returns jsonb
language sql stable as $$
  select jsonb_build_object(
    'revenue', coalesce((
